...
class supplier_info(tk.LabelFrame):
    ...
    def create_component_list(self):
        ...
        self.components.addDataGeter({'Manufacturer Part Number': lambda x: x['Parameters']['Manufacturer Part Number']})
    ...
    def create_price_order_urls(self, parent):
        from gui.widgets.price_range import price_range
        from gui.widgets.order_info import order_info as order_info_widget
        ...
        product_info_page = self.active_part['Links']['ProductInformationPage'] if 'ProductInformationPage' in self.active_part['Links'] else None
